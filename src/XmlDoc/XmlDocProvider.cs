...
namespace Kampute.DocToolkit.XmlDoc
{
    ...
    /// <remarks>
    /// <para>
    /// The <see cref="XmlDocProvider"/> is a central component in the documentation generation system that manages the
    /// loading, parsing, and resolution of XML documentation files. It serves as a repository of documentation content
    /// that can be queried by code reference or reflection member information.
    /// </para>
    /// This provider handles several key documentation processing tasks:
    /// <list type="bullet">
    ...
    ///   <item><description>Offering standardized access to documentation via code references or reflection</description></item>
    /// </list>
    /// The provider uses lazy resolution for <c>inheritdoc</c> elements, meaning these elements are only resolved
    ...
    /// <para>
    /// When handling error conditions, the provider uses a configurable error handler that can be customized through
    ...
    public class XmlDocProvider : IXmlDocProvider
    {
        ...
        /// </summary>
        /// <value>
        /// <see langword="true"/> if the provider contains no documentation; otherwise, <see langword="false"/>.
        /// </value>
        ...
        /// </summary>
        /// <value>
        /// The error handler for the documentation content provider, or <see langword="null"/> for default behavior.
        /// </value>
        /// <remarks>
        ...
        /// then imports the member documentation. If the XML document contains members that already exist in the provider,
        ...
        /// <remarks>
        /// This method adds the XML documentation of members from the provided XML document to the current provider's collection.
        ...
        {
            if (TryGetElement(cref, out var element))
            {
                doc = new XmlDocEntry(element);
                return true;
            }
        ...
        protected virtual bool TryResolveInheritedElement(XElement element, XElement inheritDoc, [NotNullWhen(true)] out XElement? inheritedElement)
        {
            ...
            if (inheritDoc.TryGetAttributeValue("cref", out var cref))
                return TryGetElement(cref, out inheritedElement);
            ...
            if (element.TryGetAttributeValue("name", out cref) && CodeReference.ResolveMember(cref) is IMember member)
            {
                ...
                if (baseMember is not null && TryGetElement(baseMember.CodeReference, out inheritedElement))
                    return true;
            }
